...
import pandas as pd
import numpy as np
from src.config import SUBSTANTIAL_SHIFT_PP
...
def generate_threshold_alerts(anomaly_df, substantial_shift_pp=SUBSTANTIAL_SHIFT_PP):
    ...
    for idx, row in df.iterrows():
        ...
        # Calibrated Commercial Alert Qualification (High Signal-to-Noise Ratio)
        # Requires either: 2+ statistical detectors firing, OR Dynamic 3-Sigma Envelope Breach (T4), OR Isolation Forest ML Anomaly
        is_qualified_shift = (stat_count >= 2) or t4 or is_iforest_anomaly or (t3 and abs(shift) >= 1.0)
        ...
        alerts.append({
            ...
            'market_share_trx_pct': row.get('market_share_trx_pct', 0.0),
            'clean_trx': row.get('brand_trx', row.get('clean_trx', 0.0)),
