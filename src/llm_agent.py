...
def answer_chatbot_question(user_prompt, context_str=""):
    ...
    # Dynamic Deterministic Fallback incorporating exact context inputs
    clean_ctx = context_str.strip() if context_str else "No context provided."
    return (
        f"**Commercial Analytics Executive Advisory**\n\n"
        f"Based on your query regarding market share and regional performance:\n"
        f"• **Executive Context:** {clean_ctx[:300]}...\n"
        f"• **Dynamic Anomaly Engine:** Multi-detector baseline evaluation ($Z \\ge 3.0$ / Scikit-Learn Isolation Forest).\n"
        f"• **Competitive Set Trajectory:** Dynamic TRx-weighted market share and share-shift trends.\n\n"
        f"*(Note: Generated via Commercial Intelligence Advisory Engine)*"
    )
